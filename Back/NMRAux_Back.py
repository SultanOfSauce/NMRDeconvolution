...
nPtsInPpm = 4096
nPts = nPtsInPpm  * ppmRange
CSRange = np.linspace(maxCSRange, minCSRange, nPts)
#Range totale frequenza = 1600Hz
...
def generateRandomSpectrum(seed = None) -> npt.ArrayLike:
    ...
    yyReturn         = np.zeros((nOff + nDyn, nPtsInPpm * ppmSubRange))
    yyFilteredReturn = np.zeros((nOff + nDyn, nPtsInPpm * ppmSubRange))
    yyRegReturn      = np.zeros((nOff + nDyn, nPtsInPpm * ppmSubRange))
    
    
    for i in range(0,nOff):
        off = rng.integers(0, (ppmRange-ppmSubRange) * nPtsInPpm)
        
        yyReturn[i,:]         = yy[off : off + ppmSubRange * nPtsInPpm] + noise[off : off + ppmSubRange * nPtsInPpm]        
        yyFilteredReturn[i,:] = yyFiltered[off : off + ppmSubRange * nPtsInPpm]     
        yyRReturn[i,:]        = yyReg[off : off + ppmSubRange * nPtsInPpm]
    ...
    for i in range(nDyn):
        ...
        yyFilteredReturn[i + nOff,:] = yyFilteredReturn[,:] * mult   
        yyRReturn[i + nOff,:]        = yyRReturn[0:]    
    
    # Peaks
    peaks, properties = find_peaks(yyNoiseless, prominence=10, width=0.1)
    wl =  fromPeaksToPos(properties["left_ips"])
    wr =  fromPeaksToPos(properties["right_ips"])
    
    xPks = CSRange[peaks]
    yPks = yyNoiseless[peaks]
    wPks = wr - wl
    yyB = np.zeros_like(CSRange)

    
    print(yy.size)
    
    return yy, yyFiltered, yyReg, {"xPks": xPks, "yPks":yPks, "wPks": wPks, "peaks": peaks}